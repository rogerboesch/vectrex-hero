...
void draw_row_tools(App *app, int px, int py, int pw, int ph) {
    ...
    snprintf(label, sizeof(label), "%s (%d / %d)",
             app->project.row_types[app->cur_row_type].name,
             app->cur_row_type + 1, app->project.row_type_count);
    ui_text(c.x, y, label);
    y += ui_line_height() + 4;
    ...
    bx += bw + gap;
    if (ui_button(bx, y, bw, ui_line_height() + 4, ">")) {
        if (app->cur_row_type < app->project.row_type_count - 1) app->cur_row_type++;
    }
    y += ui_line_height() + 12;

    /* Info */
    y = ui_section(c.x - 4, y, c.w + 8, "Info");
    RowType *rt = &app->project.row_types[app->cur_row_type];
