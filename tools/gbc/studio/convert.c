...
static void export_c(TilemapProject *tmap, const char *c_path, const char *h_path) {
    ...
    fclose(fc);  /* close main level_data.c — write per-bank files */

    /* Output one C file per bank */
    {
        char dir[512];
        strncpy(dir, c_path, sizeof(dir) - 1);
        char *slash = strrchr(dir, '/');
        if (slash) *(slash + 1) = '\0';
        else strcpy(dir, "./");

        for (int bank = 1; bank <= cur_bank; bank++) {
            char bank_path[512];
            snprintf(bank_path, sizeof(bank_path), "%slevel_data_%d.c", dir, bank);
            FILE *fb = fopen(bank_path, "w");
            if (!fb) continue;

            fprintf(fb, "// level_data_%d.c — Generated by convert tool\n\n", bank);
            fprintf(fb, "#include \"level_data.h\"\n");
            fprintf(fb, "#include <gb/gb.h>\n\n");
            fprintf(fb, "#pragma bank %d\n\n", bank);

            for (int li = 0; li < tmap->level_count; li++) {
                if (level_banks[li] != bank) continue;
                TilemapLevel *lvl = &tmap->levels[li];
                int rle_pos = rle_sizes[li];

                fprintf(fb, "BANKREF(level_%d_rle)\n", li);
                fprintf(fb, "const uint8_t level_%d_rle[] = {\n    ", li);
                for (int i = 0; i < rle_pos; i++) {
                    fprintf(fb, "0x%02X%s", rle_bufs[li][i], i < rle_pos - 1 ? "," : "");
                    if ((i & 15) == 15) fprintf(fb, "\n    ");
                }
                fprintf(fb, "\n};\n");

                fprintf(fb, "BANKREF(level_%d_row_offsets)\n", li);
                fprintf(fb, "const uint16_t level_%d_row_offsets[] = {\n    ", li);
                for (int y = 0; y < lvl->height; y++) {
                    fprintf(fb, "%d%s", row_offs_bufs[li][y], y < lvl->height - 1 ? "," : "");
                    if ((y & 15) == 15) fprintf(fb, "\n    ");
                }
                fprintf(fb, "\n};\n");

                fprintf(fb, "BANKREF(level_%d_entities)\n", li);
                fprintf(fb, "const uint8_t level_%d_entities[] = {\n    ", li);
                for (int i = 0; i < lvl->entity_count; i++) {
                    TilemapEntity *e = &lvl->entities[i];
                    fprintf(fb, "%d,%d,%d,%d%s", e->x, e->y, e->type, (uint8_t)e->vx,
                            i < lvl->entity_count - 1 ? ", " : "");
                }
                if (lvl->entity_count == 0) fprintf(fb, "0");
                fprintf(fb, "\n};\n\n");
    ...
    /* Level info table — in a separate non-banked file */
    {
        ...
        if (fm) {
            ...
            for (int li = 0; li < tmap->level_count; li++) {
                ...
                fprintf(fm, "    {%d, %d, %d, level_%d_rle, level_%d_row_offsets, %d, level_%d_entities}%s\n",
                        lvl2->width, lvl2->height, level_banks[li], li, li, lvl2->entity_count, li,
                        li < tmap->level_count - 1 ? "," : "");
            }
            fprintf(fm, "};\n");
            fclose(fm);
