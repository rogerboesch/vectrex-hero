/*
 * panel_console.c — Console log panel (same as other studios)
 */
#include "app.h"
#include "ui.h"
#include <string.h>
...
void draw_console(App *app, int px, int py, int pw, int ph) {
    ...
    int btn_w = 50, btn_x = tb.x + tb.w - btn_w;
    ...
    btn_x -= btn_w + 4;
    if (ui_button(btn_x, tb.y, btn_w, tb.h, "Clear")) app->console_count = 0;
    ...
    for (int i = start; i < app->console_count; i++) {
        ...
        else if (line[0] == '>' && line[1] == ' ') col = ui_theme.text;
