...
void app_test_level(App *app) {
    if (!app->build_dir[0]) {
        app_log_err(app, "No build_dir — save project first");
        return;
    }

    int level = app->cur_level + 1;  /* 1-based: 0 means normal startup */

    /* 1. Write test_config.c with the target level */
    {
        char path[512];
        snprintf(path, sizeof(path), "%s/test_config.c", app->build_dir);
        FILE *f = fopen(path, "w");
        if (!f) { app_log_err(app, "Cannot write %s", path); return; }
        fprintf(f, "// test_config.c — Generated by GBC Workbench\n");
        fprintf(f, "#include <stdint.h>\n\n");
        fprintf(f, "uint8_t start_level = %d;\n", level);
        fclose(f);
    }
