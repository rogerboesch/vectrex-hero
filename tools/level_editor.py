#!/usr/bin/env python3
"""
Vectrex Level & Sprite Editor

Visual editor for designing levels and VLC sprites for Vectrex games.
Supports multiple games via editor_config.json in each game folder.
Exports data as .h header files ready to #include.

Usage: python3 tools/level_editor.py --game hero
"""
...
# ---------------------------------------------------------------------------
# Game configuration — loaded from games/<game>/editor_config.json
# ---------------------------------------------------------------------------

# Defaults (H.E.R.O. compatible)
GAME_CONFIG = {
    "title": "Vectrex Game",
    "project_file": "assets/hero.json",
    "main_header": "hero.h",
    "enemy_types": ["bat", "spider", "snake"],
    "hazard_type": "lava",
    "rescue_target": "miner",
    "player_start_default": {"x": -66, "y": 40},
    "has_fuel": True,
    "has_dynamite": True,
}
...
def load_game_config(game_name):
    """Load editor_config.json for the specified game."""
    global GAME_CONFIG, GAME_NAME, GAME_DIR
    GAME_NAME = game_name
    tools_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(tools_dir)
    GAME_DIR = os.path.join(project_root, "games", game_name)
    config_path = os.path.join(GAME_DIR, "editor_config.json")
    if os.path.exists(config_path):
        with open(config_path, "r") as f:
            GAME_CONFIG.update(json.load(f))
...
"""
    ...
    def _run_level_test(self):
        ...
        main_hdr = GAME_CONFIG["main_header"]
        with open(os.path.join(src_dir, main_hdr), "r") as f:
            game_h = f.read()
        for key in ["CAVE_LEFT", "CAVE_RIGHT", "CAVE_TOP", "CAVE_FLOOR",
            ...
            if key in consts:
                game_h = re.sub(
                    ...
                    game_h)
        with open(os.path.join(test_dir, main_hdr), "w") as f:
            f.write("#define EDITOR_TEST\n" + game_h)
        ...
        lh = ["// Generated by level_editor.py — test level", "",
              "#ifndef LEVELS_H", "#define LEVELS_H", f'#include "{GAME_CONFIG["main_header"]}"', ""]
...
"""
        ...
        # ---- main.c ---- (thin wrapper; src/main.c has #ifdef EDITOR_TEST)
        mc = '#include "hero.h"\n#include "../src/main.c"\n'
    ...
    def _export_headers(self):
        default_dir = os.path.join(GAME_DIR, "src") if GAME_DIR else os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "src")
    ...
    def _export_levels_h(self, path):
        ...
        lines = [
            ...
            f"// {GAME_CONFIG['title']} level data",
            "",
            "#ifndef LEVELS_H",
            "#define LEVELS_H",
            f'#include "{GAME_CONFIG["main_header"]}"',
            "",
        ]
    ...
    def _export_sprites(self, h_path, c_path):
        ...
        h_lines = [
            "// Generated by level_editor.py",
            f"// {GAME_CONFIG['title']} VLC sprite data",
            "",
            "#ifndef SPRITES_H",
            "#define SPRITES_H",
            "",
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Vectrex Level & Sprite Editor")
    parser.add_argument('--game', default='hero', help='Game to edit (hero, cave, etc.)')
    ...
    if args.export or args.pdf:
        ...
        project_path = args.project or os.path.join(
            GAME_DIR, GAME_CONFIG["project_file"])
