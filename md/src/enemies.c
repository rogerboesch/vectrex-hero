...
void update_laser(void) {
    ...
    lx_end = laser_px + (s16)(laser_dir * LASER_LENGTH);
...
void activate_nearby_enemies(void) {
    u8 i;
    s16 ax0 = cam_x - ACTIVATE_MARGIN;
    s16 ax1 = cam_x + SCREEN_W + ACTIVATE_MARGIN;
    s16 ay0 = cam_y - ACTIVATE_MARGIN;
    s16 ay1 = cam_y + PLAY_H + ACTIVATE_MARGIN;
    ...
    for (i = 0; i < level_entity_count && active_enemy_count < MAX_ACTIVE_ENEMIES; i++) {
        ...
        s16 epx = (s16)le->tx * 8 + 4;
        s16 epy = (s16)le->ty * 8 + 4;
    ...
    for (i = 0; i < active_enemy_count; i++) {
        ...
        if (ae->type == ENEMY_SPIDER) {
            ...
            if (anim_tick & 1) {
                ...
                if (tile_solid((u8)(ae->px >> 3), (u8)((ae->py - SPIDER_HH) >> 3)) ||
                    tile_solid((u8)(ae->px >> 3), (u8)((ae->py + SPIDER_HH) >> 3))) {
                    ae->vx = -ae->vx;
                    ae->py += ae->vx;
                }
            }
            ...
            if (ae->vx > 0) {
                if (tile_solid((u8)((ae->px + SNAKE_HW) >> 3), (u8)(ae->py >> 3)))
                    ae->vx = -ae->vx;
            } else {
                if (tile_solid((u8)((ae->px - SNAKE_HW) >> 3), (u8)(ae->py >> 3)))
                    ae->vx = -ae->vx;
            ...
            if (!tile_solid((u8)(ae->px >> 3), (u8)((ae->py + SNAKE_HH + 1) >> 3))) {
                ae->vx = -ae->vx;
                ae->px += ae->vx;
            }
            ...
            // Tile collision ahead
            if (ae->vx > 0) {
                if (tile_solid((u8)((ae->px + BAT_HW) >> 3), (u8)(ae->py >> 3)))
                    ae->vx = -ae->vx;
            } else {
                if (tile_solid((u8)((ae->px - BAT_HW) >> 3), (u8)(ae->py >> 3)))
                    ae->vx = -ae->vx;
